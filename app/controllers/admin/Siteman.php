...
class Siteman extends CI_Controller {
	...
	function index()
	{
		...
		if (isset($_POST['submit'])) {
			...
			$cek = $this->model_app->cek_login($email, $password,'t_users');
			$row = $cek->row_array();
			$total = $cek->num_rows();
			if ($total > 0) {
				$array = array(
	...
	function home()
	{
		...
		$data['ses'] = $this->session->userdata();
		...
		if (isset($_POST['submit'])) {
			...
			if ($hasil['file_name']=='') {
				$data = array(
								...
								'alamat'=>cetak($this->input->post('alamat')),
								...
								'tentang'=>cetak($this->input->post('tentang')),
			...
			$this->template->load('layout/template','backend/mod_identitas/identitas',$data);
		}
	}
